...
#include "G4ParticleHPNeutronElementData.hh"
  ...
  G4ParticleHPNeutronElementData::G4ParticleHPNeutronElementData()
  {
     precision = 0.02;
 }
  ...
  void G4ParticleHPNeutronElementData::Init(G4Element * theElement, G4String dataFolder, G4int ZThreshold )
  {
    if(initialised) return;
    
    theData = new G4ParticleHPVector;
    G4int count = theElement->GetNumberOfIsotopes();
      if(count == 0) count +=
         theStableOnes.GetNumberOfIsotopes(static_cast<G4int>(theElement->GetZ()));
    theIsotopeWiseData = new G4ParticleHPXSData[count];
    initialised = true;
...
//        G4cout <<" Init: normal case"<<G4endl;
        ...
        UpdateData(A, Z, M, count++, frac, dataFolder, ZThreshold);
      }
    }else{
...
//        G4cout <<" Init: theStableOnes in the loop: "<<A<<G4endl;
    ...
    theData->ThinOut(precision);
  }  

  void G4ParticleHPNeutronElementData::UpdateData( G4int A, G4int Z, G4int M, G4int index, G4double abundance, G4String dataFolder, G4int ZThreshold ){
    theIsotopeWiseData[index].Init( A, Z, M, abundance, dataFolder, ZThreshold );
    theBuffer = theIsotopeWiseData[index].Make();
    Harmonise(theData, theBuffer);
    delete theBuffer;
  }

  void G4ParticleHPNeutronElementData::Harmonise(G4ParticleHPVector *& theStore, G4ParticleHPVector * theNew)
  {
    if(theNew == 0) { return; }
