...
class G4ParticleHPElasticDataProxy: public G4ParticleHPManagerProxyBase {
public:
  G4ParticleHPElasticDataProxy(){}
  void RegisterCrossSections( std::map<int,G4PhysicsTable *>* theCrossSections ) override {
    ...
    hpmanager->RegisterElasticCrossSections( theCrossSections );
  }
  std::map<int,G4PhysicsTable *>* GetCrossSections() override {
    G4ParticleHPManager* hpmanager = G4ParticleHPManager::GetInstance();
    return hpmanager->GetElasticCrossSections();
  }
};

G4ParticleHPElasticData::G4ParticleHPElasticData() 
   :G4ParticleHPNeutronXSDataSet("NeutronHPElasticXS",MODEL_NAME),
   theMgrProxy()
{
   theMgrProxy.reset( new G4ParticleHPElasticDataProxy );
...
std::vector<std::pair<std::string,G4ParticleHPNeutronData > >& G4ParticleHPElasticData::dataStore(){
