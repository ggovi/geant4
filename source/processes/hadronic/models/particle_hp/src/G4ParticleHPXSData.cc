...
G4bool G4ParticleHPXSData::Init(G4int A, G4int Z, G4int M, G4double abun, G4String dirName, G4String aFSType)
  {
    theChannelData = 0;
...
#ifdef G4PHPDEBUG
    if(std::getenv("G4ParticleHPDebug")) G4cout << "G4ParticleHPXSData::Init = "<< filename <<" "<< A << " " << Z <<G4endl;
#endif
...
//    theChannel.close();
  ...
  void G4ParticleHPXSData::Init( G4int A, G4int Z, G4int M, G4double abun, G4String dataFolder, G4int ZThreshold ){
    theChannelData = 0;
    bool ok = false;
    if(ZThreshold==0 || Z>ZThreshold){ 
      ok = Init(A, Z, M, abun, dataFolder, "/CrossSection");
    }
    if(!ok){
      theChannelData = new G4ParticleHPVector;
    }
    theData = theChannelData;
  }
  
  G4String G4ParticleHPXSData::GetName(G4int A, G4int Z, G4String base, G4String rest)
  {
    G4bool dbool;
